...
int _open(const char *name, int flags, int mode) {
   ...
   {
      /* DDR descriptor lookup: check if ARM pre-loaded this file into DDR.
       * ARM writes descriptor at DDR_DESCRIPTOR_BASE before releasing VexRiscv.
       * Supports any model/file without firmware rebuild. */
      {
         volatile DdrDescriptor *desc = (volatile DdrDescriptor *)DDR_DESCRIPTOR_BASE;
         if(desc->magic == DDR_DESCRIPTOR_MAGIC) {
            uint32_t j;
            for(j = 0; j < desc->num_files && j < DDR_DESCRIPTOR_MAX_FILES; j++) {
               if(strcmp(name, (const char*)desc->files[j].name) == 0) {
                  files[i].status = true;
                  files[i].curr   = 0;
                  files[i].len    = desc->files[j].size;
                  files[i].body   = (const uint8_t*)desc->files[j].ddr_addr;
                  return i + FP_FIRST;
               }
            }
         }
      }
      /* DDR lookup found nothing — fall back to SD card via FatFs */
      if(!usd_is_init) {
...
/* DDR ring buffer output — avoids spinning on broken APB UART.
 * ARM loader polls DDR_PRINT_BUF_BASE and forwards to PS UART. */
#define DDR_PRINT_BUF_BASE  0x0C7F0000
#define DDR_PRINT_BUF_SIZE  4096
#define DDR_PRINT_MAGIC     0x50524E54  /* "PRNT" */
/* Layout: [magic:4][wr_idx:4][rd_idx:4][data:DDR_PRINT_BUF_SIZE] */
static volatile uint32_t *_pbuf_magic   = (volatile uint32_t *)DDR_PRINT_BUF_BASE;
static volatile uint32_t *_pbuf_wr      = (volatile uint32_t *)(DDR_PRINT_BUF_BASE+4);
static volatile uint32_t *_pbuf_rd      = (volatile uint32_t *)(DDR_PRINT_BUF_BASE+8);
static volatile uint8_t  *_pbuf_data    = (volatile uint8_t  *)(DDR_PRINT_BUF_BASE+12);

ssize_t _write(int fd, const void *ptr, size_t len) {
   if(fd == STDOUT_FILENO || fd == STDERR_FILENO) {
      int i;
      char *p;
      if(*_pbuf_magic != DDR_PRINT_MAGIC) {
         *_pbuf_wr    = 0;
         *_pbuf_rd    = 0;
         *_pbuf_magic = DDR_PRINT_MAGIC;
      }
      for(i=0,p=(char *)ptr;i < len;i++,p++) {
         uint32_t next = (*_pbuf_wr + 1) % DDR_PRINT_BUF_SIZE;
         /* Wait if buffer full */
         while(next == *_pbuf_rd);
         _pbuf_data[*_pbuf_wr] = (uint8_t)*p;
         *_pbuf_wr = next;
      }
      return len;
   }
