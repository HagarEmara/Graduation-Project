...
function safe_log2(x:integer) return integer is
begin
   if x <= 1 then return 0;
   else return log2(x);
   end if;
end function;
constant SCALE_FACTOR_L:integer:=safe_log2(SCALE_FACTOR);
...
aximaster_arsize_out <= std_logic_vector(to_unsigned(MASTER_DATA_BYTE_WIDTH_L,3));
...
begin
   ...
   axislave_cmd_rec_write.arsize <= std_logic_vector(to_unsigned(MASTER_DATA_BYTE_WIDTH_L,3)); -- axislave_arsize_in;
...
variable step_v:unsigned(0 downto 0);
begin
   step_v := (others=>'0');
   ...
   if(axislave_resp_fifo_empty='0') then
      ...
      if SCALE_FACTOR > 1 then
         ...
         rdata(SLAVE_DATA_WIDTH-1 downto 0) <= axislave_resp_rec_read.rdata(SLAVE_DATA_WIDTH-1 downto 0);
         axislave_resp_fifo_rd <= rready;
      end if;
